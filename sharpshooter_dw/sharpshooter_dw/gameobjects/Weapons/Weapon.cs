...
namespace sharpshooter_dw
{
    public abstract class Weapon
    {
        ...
        public void fire(Soldier personfiring)
        {
            ...
            bullet.location.X = this.location.X + (float)Math.Cos(facingangle / 180f * Math.PI) * bulletstartdistance;
            bullet.location.Y = this.location.Y + (float)-Math.Sin(facingangle / 180f * Math.PI) * bulletstartdistance;


            //TASK 2: Giving the bullet forward velocity based on the current angle 

            bullet.velocity.X = (float)Math.Cos(facingangle * Math.PI / 180) * bulletspeed;
            bullet.velocity.Y = (float)-Math.Sin(facingangle * Math.PI / 180) * bulletspeed;
        }
